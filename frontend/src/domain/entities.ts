/**
 * domain/entities: SPINE §4.2 Entity TypeScript mirror.
 *
 ...
 *
 * Domain invariants (SPINE §4 / DATA-MODEL §2 / §7):
...
/**
 * Author identifier value object. Embedded in snapshot author. (SPINE §4.2 TeamIdentity)
 */
...
/**
 * Session line (code git branch name reused). (SPINE §4.2 Branch)
 *
...
/**
 * commit (immutable, content-addressed). (SPINE §4.2 Snapshot / DATA-MODEL §2.2)
 *
 ...
 * → Same body + different meta can share the same id (DATA-MODEL §2.2, OQ-1).
 *
...
/**
 * Variable pointer (HEAD / branch / tag unified representation). (SPINE §4.2 Ref / DATA-MODEL §2.3)
 *
 * Three kinds:
...
/**
 * CIR container — normalized conversation body (immutable). (SPINE §4.2 SessionDoc)
 *
...
/**
 * Distilled memory (for memory-form load). (SPINE §4.2 MemoryDigest)
 *
...
/**
 * Repository unit metadata index — catalog for push/pull negotiations. (SPINE §4.2 Manifest / DATA-MODEL §4)
 *
 * Invariants:
