...
func TestInviteNoPrivilegeEscalation(t *testing.T) {
	...
	repositoryRecord := domain.Repository{ID: domain.NewID("ws_"), Name: "T", OwnerID: owner.ID, Slug: "t", OwnerUsername: "owner", CreatedAt: time.Now().UTC()}
	if err := fs.CreateRepository(ctx, repositoryRecord); err != nil {
		t.Fatal(err)
	}
	if err := fs.AddMember(ctx, domain.Membership{RepositoryID: repositoryRecord.ID, UserID: owner.ID, Role: domain.RoleOwner, CreatedAt: time.Now().UTC()}); err != nil {
		t.Fatal(err)
	}
	if err := fs.AddMember(ctx, domain.Membership{RepositoryID: repositoryRecord.ID, UserID: maint.ID, Role: domain.RoleMaintainer, CreatedAt: time.Now().UTC()}); err != nil {
		t.Fatal(err)
	}

	// Maintainer attempts to invite owner → rejected (privilege escalation block — critical regression).
	if _, err := svc.Invite(ctx, maint.ID, repositoryRecord.ID, "", domain.RoleOwner, 0); !errors.Is(err, domain.ErrForbidden) {
	...
	if _, err := svc.Invite(ctx, maint.ID, repositoryRecord.ID, "", domain.RoleMember, 0); err != nil {
		t.Fatalf("member invitation by maintainer rejected: %v", err)
	}
	// Invalid role → silent member downgrade is replaced by ErrValidation.
	if _, err := svc.Invite(ctx, owner.ID, repositoryRecord.ID, "", domain.MemberRole("superadmin"), 0); !errors.Is(err, domain.ErrValidation) {
		t.Fatalf("invalid role not rejected: err=%v", err)
	}
	// Owners can invite other owners (co-owner assignment requires owner privileges).
	if _, err := svc.Invite(ctx, owner.ID, repositoryRecord.ID, "", domain.RoleOwner, 0); err != nil {
	...
	if _, err := svc.Invite(ctx, owner.ID, repositoryRecord.ID, "", domain.RoleMember, -time.Hour); !errors.Is(err, domain.ErrValidation) {
		t.Fatalf("negative ttl not rejected: err=%v", err)
	}
	inv, err := svc.Invite(ctx, owner.ID, repositoryRecord.ID, "", domain.RoleMember, time.Millisecond)
...
func TestAcceptInviteNeverDowngradesExistingMember(t *testing.T) {
	...
	repositoryRecord := domain.Repository{ID: domain.NewID("ws_"), Name: "Roles", OwnerID: owner.ID, Slug: "roles", OwnerUsername: owner.Username}
	if err := fs.CreateRepository(ctx, repositoryRecord); err != nil {
		t.Fatal(err)
	}
	if err := fs.AddMember(ctx, domain.Membership{RepositoryID: repositoryRecord.ID, UserID: owner.ID, Role: domain.RoleOwner}); err != nil {
		t.Fatal(err)
	}
	if err := fs.AddMember(ctx, domain.Membership{RepositoryID: repositoryRecord.ID, UserID: member.ID, Role: domain.RoleMaintainer}); err != nil {
		t.Fatal(err)
	}
	oldViewerInvite, err := svc.Invite(ctx, owner.ID, repositoryRecord.ID, member.Email, domain.RoleViewer, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	if role, ok := svc.RoleOf(ctx, repositoryRecord.ID, member.ID); !ok || role != domain.RoleMaintainer {
...
func TestRevokeInviteCannotCrossRepositoryBoundary(t *testing.T) {
	...
	repositoryA := domain.Repository{ID: domain.NewID("ws_"), Name: "A", OwnerID: ownerA.ID, Slug: "a", OwnerUsername: ownerA.Username}
	repositoryB := domain.Repository{ID: domain.NewID("ws_"), Name: "B", OwnerID: ownerB.ID, Slug: "b", OwnerUsername: ownerB.Username}
	for _, repositoryRecord := range []domain.Repository{repositoryA, repositoryB} {
		if err := fs.CreateRepository(ctx, repositoryRecord); err != nil {
			t.Fatal(err)
		}
	}
	if err := fs.AddMember(ctx, domain.Membership{RepositoryID: repositoryA.ID, UserID: maintA.ID, Role: domain.RoleMaintainer}); err != nil {
		t.Fatal(err)
	}
	invB, err := svc.Invite(ctx, ownerB.ID, repositoryB.ID, "", domain.RoleMember, 0)
	if err != nil {
		t.Fatal(err)
	}
	if err := svc.RevokeInvite(ctx, maintA.ID, repositoryA.ID, invB.Token); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("cross-repository revoke error = %v", err)
	}
