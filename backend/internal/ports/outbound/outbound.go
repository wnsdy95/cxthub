...
//   - AuthProvider:  team token ↔ team mapping, repo visibility (SYNC-PROTOCOL §4·§6).
...
//
// The body (CIR doc / memory) is not stored here but in BlobStore (meta/body separation, DATA-MODEL §6.2).
...
type MetadataStore interface {
	...
	// SetSnapshotMemory attaches a memory pointer (memory_hash) to the snapshot — after a snapshot push, reflecting memorize (RECONCILIATION E: raw+memory co-possess). ID remains unchanged.
	...
	// Memory meta (derivative; body is BlobStore). Target snapshot must exist (SYNC-PROTOCOL §2.7).
...
// BlobStore stores content-addressed immutable bodies (DATA-MODEL §6.1).
//
...
//
// Storage units: SessionDoc (CIR body) and MemoryDigest body. Both have sha256:<hex> keys. Integrity: Recalculate hash on Get to verify key (SYNC-PROTOCOL §3.4, verification is in impl).
...
// AuthProvider validates team tokens and determines visibility boundaries (SYNC-PROTOCOL §4·§6).
//
...
// GitEngine is a git engine (_ARCHITECTURE-R2 §2: backend has).
//
// It performs commit/branch/fork/diff/ref(HEAD/tag) based on CIR/hash, independent of provider raw format.
// Core responsibility is DAG reachability (fast-forward determination, LCA, parents transitive closure) (SYNC-PROTOCOL §3.1·§5.2, DATA-MODEL §2.5).
...
type GitEngine interface {
	...
	// AncestorsClosure: Parents transitive closure of ids (includes missing ancestors, SYNC-PROTOCOL §3.1).
	...
	// VerifyIntegrity: Invariant verification (Snapshot.ID == ContentHash(canonical_bytes(doc.CIR))) (§3.4).
...
// RefMoveClass is the classification returned by ClassifyRefMove (SYNC-PROTOCOL §5.1).
