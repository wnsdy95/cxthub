...
//
// Under the complete-separation rule (docs/_ARCHITECTURE-R2.md §5), the backend shares no Go code
// with the CLI. Wire/JSON shapes derive from schemas/cir.schema.json, schemas/manifest.schema.json,
// docs/DATA-MODEL.md, and docs/SYNC-PROTOCOL.md.
//
...
// (docs/DATA-MODEL.md §3.1, schemas/manifest.schema.json $defs/contentHash).
//
// Determinism requires identical logical content to produce identical hex. Canonical serialization
// (canonical_bytes) before hashing provides that property (DATA-MODEL §3.1, RFC 8785 JCS-oriented).
...
const (
	...
	// ProviderUnknown represents an unknown provider (the third enum value required by RECONCILIATION §E).
...
// HeadRefName is the fixed head-ref name (invariant REF3, DATA-MODEL §2.3).
...
// TeamIdentity identifies a snapshot author (DATA-MODEL §4 table, SYNC-PROTOCOL §2.4).
//
// Unlike Git's separate author and committer, cxt records a single Author (OQ-7).
// It is for attribution and auditing, not access control (SYNC-PROTOCOL §6).
