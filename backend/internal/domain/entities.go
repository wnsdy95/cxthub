...
// Repo is the root of the session storage space (DATA-MODEL §2.1, SYNC-PROTOCOL §2.1).
//
...
// LocalPath is always an empty string on the server (local-only field, SYNC-PROTOCOL §2.1).
type Repo struct {
...
// MemoryHash, GraftParents, and GraftSeq are updated under their own merge/CAS rules (DATA-MODEL §2.2).
//
...
// The server recomputes and verifies this invariant when it receives a push (SYNC-PROTOCOL §3.4).
//
// Invariants:
...
//
// MemoryHash is a pointer to the MemoryDigest attached to the raw doc (RECONCILIATION §E, optional, "" possible).
// Snapshots can hold both raw (DocHash) and memory (MemoryHash) together (PRIMARY-USE-CASE invariant 1).
...
// (DATA-MODEL §2.3, SYNC-PROTOCOL §2.3).
//
...
// Ref is a mutable name pointing to a snapshot (DATA-MODEL §2.3, schemas/manifest.schema.json $defs/ref).
//
...
// Manifest is the repository-level metadata index (DATA-MODEL §4, schemas/manifest.schema.json).
//
...
//   - M2: Version must monotonically increase with each update (§5.1 CAS).
...
// MemoryDigest is the distilled memory from a snapshot (derivative, DATA-MODEL §4 table, SYNC-PROTOCOL §2.7).
//
// Used in memory-form load (FidelityMemory). Attached to SnapshotID, and the target snapshot must exist before the digest upload (422 if not, SYNC-PROTOCOL §2.7).
//
