...
// recalculating the canonical_bytes hash for integrity verification (SYNC-PROTOCOL §3.4).
...
//
// Note (DATA-MODEL §3.1 OQ-5): Deterministic metadata like CapturedAt/SessionOriginID, if included in canonical_bytes,
...
// (canonical_bytes rule, DATA-MODEL §3.1).
type CIREvent struct {
	...
	ToolName         string         `json:"tool_name,omitempty"`          // Standard Name (CROSS-COMPAT §2.1 Source of Truth)
...
// (CROSS-COMPAT §3.3: opaque preservation, no interpretation, no cross-injection).
...
// (DATA-MODEL §2 Table, SYNC-PROTOCOL §2.5). Exposed in the wire as docs/{hash}.
//
