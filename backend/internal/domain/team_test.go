...
func TestEffectiveRepositoryRoleRequiresCurrentOrganizationAndTeamMembership(t *testing.T) {
	...
	if role, ok := EffectiveRepositoryRole(repository, nil, "reader", []TeamRepositoryAccess{grant}, OrganizationRepositoryAccess{}); !ok || role != RoleMember {
	...
	for _, change := range []func(*TeamRepositoryAccess){
		...
		bad := grant
		change(&bad)
		if _, ok := EffectiveRepositoryRole(repository, nil, "reader", []TeamRepositoryAccess{bad}, OrganizationRepositoryAccess{}); ok {
	...
	if role, ok := EffectiveRepositoryRole(repository, direct, "reader", []TeamRepositoryAccess{grant}, OrganizationRepositoryAccess{}); !ok || role != RoleMaintainer {
		t.Fatal("team grant reduced an explicit direct grant")
	}
	if _, ok := EffectiveRepositoryRole(repository, nil, "", []TeamRepositoryAccess{grant}, OrganizationRepositoryAccess{}); ok {
