...
// PutRepo is idempotent: if it already exists, it returns the existing record (SYNC-PROTOCOL §2.1). Exception: for unowned (workspace_id="") records, it fills in a delayed binding if a new binding arrives.
...
func (s *FSStore) PutMemoryMeta(ctx context.Context, repoID domain.ContentHash, digest domain.MemoryDigest) error {
	...
	// Target snapshot preexistence required (SYNC-PROTOCOL §2.7).
