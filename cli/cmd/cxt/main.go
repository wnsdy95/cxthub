...
// (SPINE §3.2 Rule 5: cmd/cxt can import everything. DI is only here.)
//
...
//
// Subcommands (SPINE §7.2, client-specific):
//
...
func run(args []string) error {
	...
	case "hook":
		// hook safety contract (CAPTURE §2.5): capture failures must not block agent sessions —
...
// DI is performed only within this function (SPINE §3.2 Rule 5).
func buildContainer(cfg config) container {
	...
	// Memory adapter (_RECONCILIATION B section): MemorySource/MemorySink registry + self-distillation MemoryDistiller.
	...
	// Session materializer(_RECONCILIATION C.1): full-context recovery native resume synthesis.
