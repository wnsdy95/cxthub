...
// CROSS-COMPAT §2.1 This is the canonical tool_name source of truth (_RECONCILIATION F section).
...
// Canonical tool_name vocabulary (canonical, _RECONCILIATION F section):
//
...
// canonicalToolName converts the provider original tool name to the CIR canonical tool name (CROSS-COMPAT §2.1 = source of truth).
//
...
func canonicalToolName(providerName string) string {
	...
	// Unknown tool: preserved as unknown:<original_name> (CROSS-COMPAT §2.3)
...
//
// Note (CROSS-COMPAT §2.2/§2.3): For the same provider encoding, the preserved provider_tool_name is prioritized to ensure lossless conversion,
...
func providerToolName(provider domain.ProviderKind, canonical string) string {
	...
	case domain.ProviderCodex:
		...
		case toolShell, toolReadFile, toolListDir, toolGrep:
			return "shell" // codex is promoted to shell for file operations (CROSS-COMPAT §2.3)
		case toolApplyPatch:
			return "apply_patch"
		case toolUpdatePlan:
			return "update_plan"
