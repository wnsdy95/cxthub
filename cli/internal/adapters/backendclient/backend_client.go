...
// BackendClient implements RemoteSync for synchronous REST sync with the central server (SYNC-PROTOCOL §2/§3).
//
// Uses net/http (stdlib). baseURL is the central server REST base (e.g., http://127.0.0.1:8080/api/v1).
// Attaches Authorization: Bearer + X-Cxt-Identity header to all requests (§4).
...
// Push performs three steps: negotiate(A) → objects(B) → refs PUT(C) for uploading only missing parts (SYNC-PROTOCOL §3.2).
