// Package cli contains the cxt CLI driver (SPINE §7.2).
//
...
func Run(c *Container, args []string) error {
	...
	default:
		return fmt.Errorf("%q: unknown command. Supported: setup|init|remote|add|commit|save|list|switch|checkout|fork|load|stash|push|pull|memorize|tag|config|login|logout|secrets|settings|hooks|fsck|reflog|mcp", cmd)
	}
}
...
func printUsage() {
	fmt.Println(`cxt — Git-style version control for coding-agent sessions
usage: cxt <command> [flags]
  ...
  init                      initialize the local context store and install Git hooks (--no-hooks to skip hooks)
  remote add origin <url>   connect the server repository URL (the context equivalent of Git origin)
  add [claude|codex]        stage a session for the next commit (defaults to Claude)
  ...
  push | pull [--force]     synchronize with origin (also run automatically by Git push/pull; first push creates the repo)
                            push rejects non-fast-forward updates; pull stops on conflicts and keeps local state
  tag [<name> [ref]]        list or create immutable tags (equivalent to Git tags)
  memorize                  distill the head context into reusable memory (included in the next push)
  settings pull|list|restore  apply team defaults, list backups, or restore a backup
  secrets push|pull -p <pw> share .cxtsecrets with end-to-end encryption (the server never sees plaintext)
  stash [push|pop|list]     save or restore a session (equivalent to Git stash; also run automatically)

  cxt-specific commands:
  save [-m msg] [--provider claude|codex]   create a snapshot only (unlike commit, this does not resolve
                            pending state or synchronize; use commit to share and clear web in-progress state)
  list | log [--branch B]   snapshot list
  fork <ref> --as <branch>  fork + restore
  load <ref> [--mode full|memory]           restore only
  remote [-v] | remote remove <name>        remote management
  hooks install|uninstall   manage Git hooks manually
  fsck | reflog             audit server integrity or view the ref-move log
  repack                    reclaim duplicate prefix storage through chunk CAS
  mcp                       read-only MCP server (stdio) — agent queries team context
                            (tool: context_list/context_fetch/memory_load/context_search)
  config checkout.mode [auto|prepare]       context behavior on Git branch changes (default: auto)`)
}
