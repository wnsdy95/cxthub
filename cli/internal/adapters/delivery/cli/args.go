...
var commandArgSpecs = map[string]commandArgSpec{
	...
	"memory":    {usage: "cxt memory [<ref>] [--provider claude|codex] [--claims <json-file>]", flags: commandFlags([]string{"--provider", "--claims"}, nil)},
...
func validateCommandFlags(cmd string, args []string, spec commandArgSpec) error {
	...
	case "branch":
		...
		if flagPresent(args, "--json") {
			return fmt.Errorf("branch: --json is only valid with operations")
		}
