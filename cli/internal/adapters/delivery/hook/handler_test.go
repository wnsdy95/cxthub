...
// TestHandlerStopWithPayload fixes the flow where Stop capture continues from the transcript path/cwd (CAPTURE §2.4 — detection omission path).
...
// TestHandlerGarbagePayload ensures it doesn't die on non-JSON stdin (§10-C best-effort).
