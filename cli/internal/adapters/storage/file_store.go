...
//
// It corresponds to a self-contained object DB for the .git directory (USER DECISION 2, _RECONCILIATION C.3). It adopts git's mental model of content-addressing: same bytes ⇒ same sha256 ⇒ automatic dedup.
//
...
func (s *FileStore) PutSnapshot(_ context.Context, snap domain.Snapshot) error {
	...
	if fileExists(p) {
		...
		//   - MemoryHash attachment (derived pointer, RECONCILIATION section): attaches if new value.
