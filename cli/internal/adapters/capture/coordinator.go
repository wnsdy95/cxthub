...
// CaptureCoordinator is a common adjuster for auto/manual capture (CAPTURE §2.3).
//
...
//
// State files (local-only, re-creatable, push non-target — CAPTURE §8 simplified by repo unit):
//
...
//
// Safety contract (invariant, CAPTURE §2.5):
...
func acquireLock(path string) bool {
	for i := 0; i < 2; i++ {
		...
		if serr != nil || time.Since(fi.ModTime()) < staleLockAge {
			return false // another process is capturing — no-op (CAPTURE §4.4)
		}
...
// MarkBaseline records the baseline state at the SessionStart event (CAPTURE §2.2). Does not commit.
...
// MarkTurn records the turn boundary at the UserPromptSubmit event (CAPTURE §2.2). Does not commit.
...
// RequestCapture requests a capture (CAPTURE §2.3 commit/flush path).
// If debounce=true, applies the marker file mtime gate (§4.1 leading + mtime gate),
// and if force=true (SessionEnd flush/manual), ignores debouncing. Growth gate (§4.2 cheap gate) and
// file lock (§4.4) are always applied. Final dedup is by Save's content-hash dedup.
	...
	if path == "" {
		...
		if path, serr = src.LocateActiveSession(ctx, cwd); serr != nil {
			if errors.Is(serr, domain.ErrNoActiveSession) {
				return false, nil // inactive no-op (§2.5)
			}
			return false, serr
		}
	}
