...
const (
	...
	// ProviderUnknown is an unidentified provider (_RECONCILIATION section).
...
// HookMessagePrefix is the message prefix of snapshots created by agent hook auto-capture (CAPTURE §2). It is used for sliding checkpoint determination (replacing the tip of a hook snapshot with a new capture) — a vocabulary for identifying hook origin without adding domain fields (hash invariant).
const HookMessagePrefix = "hook: "

// FidelityTier is the fidelity tier of session load/recovery (SPINE §5.5). Allowed values: "full", "reconstructed", "memory".
...
// RefKind indicates the type of variable pointer (SPINE §4.1). Allowed values: "head", "branch", "session", "tag".
type RefKind = string
