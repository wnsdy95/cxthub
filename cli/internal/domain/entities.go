...
// TeamIdentity is a value object that identifies the snapshot author (SPINE §4.2).
...
// Repo is the root of the session storage space (SPINE §4.2). One Repo per code repository.
type Repo struct {
...
// Branch represents the session line (code git branch name, SPINE §4.2).
type Branch struct {
...
// Ref is a mutable pointer (HEAD/branch/session/tag unified representation, SPINE §4.2).
type Ref struct {
...
// SessionDoc is a CIR container (regular conversation body, immutable, SPINE §4.2).
//
// Invariant: Hash == ContentHash(canonical_bytes(CIR)).
type SessionDoc struct {
	// Hash is the ContentHash of this SessionDoc.
	Hash ContentHash `json:"hash"`
	// CIR is a provider-independent regular conversation representation (SPINE §5).
	CIR CIRDocument `json:"cir"`
}

// MemoryDigest is the distilled memory (memory-form load, SPINE §4.2).
...
// NativeMemory is a native memory value object created by the provider CLI (Section _RECONCILIATION B).
...
// Manifest is a repo unit metadata index (snapshot/ref list catalog, SPINE §4.2).
...
// Pending is a session-specific "in-progress context" mutable pointer (CAPTURE §2.2 Hook Auto-Capture Anchor).
