...
// SessionStore is a content-addressed snapshot/document/ref persistence port (SPINE §6.1).
//
...
type SessionStore interface {
	...
	// If not found, returns domain.ErrNotFound. (_RECONCILIATION E clause)
	GetMemory(ctx context.Context, hash domain.ContentHash) (domain.MemoryDigest, error)
}

// ProviderCodec is a codec for provider raw JSONL ↔ CIR bidirectional transformation (SPINE §6.1).
//
...
// CaptureSource finds and reads the active session file based on the current working directory (SPINE §6.1).
//
...
// GitContext is a port for querying the current code repo / branch (SPINE §6.1).
//
...
type RemoteSync interface {
	...
	// Call before push to ensure the server holds metadata like remote URL (SYNC-PROTOCOL §2.1).
...
// MemorySource is a port that absorbs provider native memory (_RECONCILIATION B section).
//
...
// MemoryDistiller is a port that generates MemoryDigest (summary memory) (_RECONCILIATION B section).
//
...
// MemorySink is a port that injects MemoryDigest into the target provider native memory file (_RECONCILIATION B section).
//
// Writes to Claude CLAUDE.md/MEMORY.md, Codex AGENTS.md, etc. Does not hijack native directories.
// Records only at sink time during load (_RECONCILIATION C.3).
